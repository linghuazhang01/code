...
然后对 response tokens 做 masked mean，得到 sample-level OPD loss。

| Metric | 含义 | 计算方式 |
| --- | --- | --- |
| `<domain>/loss/opd_loss_mean` | 该 domain 的 sample-level OPD signal 平均值。 | 先对每个样本做 `masked_mean(reverse_kl)`，再对 domain 内样本求均值。 |
| `<domain>/loss/opd_loss_variance` | 该 domain 内 sample-level OPD signal 方差。 | `var(sample_loss_mean in domain)`。 |
| `<domain>/loss/sample_loss_variance_mean` | 单样本 token-level OPD signal 方差的 domain 平均值。 | 每个样本先算 `masked_mean(reverse_kl^2) - masked_mean(reverse_kl)^2`，再对 domain 内样本求均值。 |
...
| `loss_variance_sample.jsonl` | 每 step、每 train domain 最多 `max_samples_per_domain` 条样本级 `sample_loss_mean` 和 `sample_loss_variance`。 |
