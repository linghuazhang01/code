...
重要边界：`pre_reweight_full_grad` 指**当前 actor update mini-batch 在 dynamic domain reweight 之前、按 train domain 分解的完整 actor 参数梯度**。这些梯度来自 audit backward replay，使用 0/1 domain mask，不包含 `dynamic_domain_loss_weighting` 的 domain weight。实际训练 backward 使用 reweight 后的 gradient mask；其一致性由 `global/full_grad_training_parity/*` 检查。

核心实现位置：

- `code/mopd_verl/verl_audit.py`：每个 training step 的 loss / teacher / calibration / coverage audit。
- `code/mopd_verl/domain_gradient/audit.py`：pre-reweight full-parameter domain gradient、token-gradient 与 dynamic weighting。
- `code/mopd_verl/domain_gradient/geometry.py`：gradient norm、alignment、conflict、closure 与 training parity。
...
global/pre_reweight_full_grad_conflict/math_vs_code/pre_reweight_full_grad_cosine_train_i_k
global/pre_reweight_full_grad_alignment/math_vs_pre_reweight_total/pre_reweight_full_grad_cosine_domain_total
global/pre_reweight_full_grad_contribution/math_to_pre_reweight_total/signed_projection_share
...
训练 actor 会返回当前配置实际使用的 token-level distillation
loss 和对应的 active-loss mask。其具体形式由
`distill_loss_builder` / `distill_mode` 决定，例如 chosen-token
reverse-KL、policy-gradient distillation signal 或 Top-k KL。Top-k
模式还包含配置的 loss weight 与实际应用的 detached rollout IS
weight；teacher-prefix 模式分别记录 prefix forward-KL 与 suffix
distillation loss。未参与 loss 的 token 会被 active mask 排除。

当 `ppo_epochs > 1` 时，同一 token 在各 PPO epoch 的 loss 先求
mean，再进入 audit 统计。因此 `loss_epoch_reduction=mean`，
`loss_epoch_count` 为实际 PPO epoch 数。

chosen-token reverse-KL 的基础形式为：
...
仅对缺少 actor-returned configured loss 的 legacy caller，audit 才使用
上述 chosen-token reverse-KL fallback，并明确标记
`loss_name=chosen_token_reverse_kl_fallback`、
`loss_epoch_reduction=single_forward`。`critic_warmup` 中没有 actor
update 的 step 不生成 actor-loss audit row。

然后分别在 token-level 和 sample-level 做统计，且两类统计都会输出
global 与 per-domain 指标：

- token-level：把当前 scope 内所有 active-loss tokens 的
  `configured_token_loss` 展平后计算
  mean / std / variance / p05 / p50 / p95 / max / sum。
- sample-level：每个样本先累加
  `sum(configured_token_loss * configured_token_loss_mask)`，再在当前
  scope 的样本维度计算 mean / std / variance。

| Metric | 含义 | 计算方式 |
| --- | --- | --- |
| `<domain>/loss/token_opd_loss_mean` | 当前 domain 内 token-level configured loss 均值。 | 对该 domain 的全部 active-loss tokens 计算 mean。 |
| `<domain>/loss/token_opd_loss_std` | 当前 domain 内 token-level configured loss 标准差。 | 对该 domain 的全部 active-loss tokens 计算 std。 |
| `<domain>/loss/token_opd_loss_variance` | 当前 domain 内 token-level configured loss 方差。 | 对该 domain 的全部 active-loss tokens 计算 `mean(loss^2)-mean(loss)^2`。 |
| `<domain>/loss/token_opd_loss_{p05,p50,p95,max,sum}` | 当前 domain 内 token-level configured loss 的分位数、最大值与总和。 | 对 active-loss token vector 直接统计。 |
| `<domain>/loss/sample_opd_loss_mean` | 当前 domain 内 sample-level configured loss 均值。 | 每个样本先按 active mask 求 token loss sum，再对该 domain 样本求 mean。 |
| `<domain>/loss/sample_opd_loss_std` | 当前 domain 内 sample-level configured loss 标准差。 | 每个样本先按 active mask 求 token loss sum，再对该 domain 样本求 std。 |
| `<domain>/loss/sample_opd_loss_variance` | 当前 domain 内 sample-level configured loss 方差。 | 每个样本先按 active mask 求 token loss sum，再对该 domain 样本求 variance。 |
| `global/loss/token_opd_loss_mean` | 当前 batch 全局 token-level configured loss 均值。 | 对全 batch 所有 active-loss tokens 计算 mean。 |
| `global/loss/token_opd_loss_std` | 当前 batch 全局 token-level configured loss 标准差。 | 对全 batch 所有 active-loss tokens 计算 std。 |
| `global/loss/token_opd_loss_variance` | 当前 batch 全局 token-level OPD loss 方差。 | 对全 batch 所有有效 response tokens 计算 variance。 |
| `global/loss/sample_opd_loss_mean` | 当前 batch 全局 sample-level configured loss 均值。 | 每个样本先按 active mask 求 token loss sum，再对全 batch 样本求 mean。 |
| `global/loss/sample_opd_loss_std` | 当前 batch 全局 sample-level configured loss 标准差。 | 每个样本先按 active mask 求 token loss sum，再对全 batch 样本求 std。 |
| `global/loss/sample_opd_loss_variance` | 当前 batch 全局 sample-level configured loss 方差。 | 每个样本先按 active mask 求 token loss sum，再对全 batch 样本求 variance。 |
...
其中 `full_gradient_train_max_samples_per_domain: null` 表示 train probe 使用当前 actor update mini-batch 内该 domain 的全部样本；多 mini-batch 时由 verl 原有 metrics reducer 做均值聚合。token-gradient audit 的 candidate pool 是当前 step 内该 domain 的全部 valid response tokens，不再做 per-sample top-k 截断。`token_gradient_top_k: null` 会关闭 Top-k selection/replay，但不会关闭 Tail 或 Top-p；正整数则启用对应 Top-k。`token_gradient_enabled` 是总开关。
...
| Metric | 含义 | 计算方式 |
| --- | --- | --- |
| `<train_domain>/pre_reweight_full_grad/grad_norm` | dynamic domain reweight 前，该 train domain 的参数梯度范数。 | 用 0/1 domain mask replay 后计算 `||g_i||_2`。 |
| `<train_domain>/pre_reweight_full_grad/sample_count` | pre-reweight domain replay 覆盖的样本数。 | 当前 actor update mini-batch 中该 domain 样本数，经 distributed sum。 |
| `global/pre_reweight_full_grad/total_grad_norm` | reweight 前所有 domain gradient 之和的范数。 | `||sum_i g_i||_2`。 |
| `global/pre_reweight_full_grad_conflict/<domain_i>_vs_<domain_k>/pre_reweight_full_grad_cosine_train_i_k` | reweight 前两个 train domains 的参数梯度方向相似度。 | `cosine(g_i, g_k)`。 |
| `global/pre_reweight_full_grad_alignment/<domain_i>_vs_pre_reweight_total/pre_reweight_full_grad_cosine_domain_total` | reweight 前 domain gradient 与 pre-reweight total 的方向一致性。 | `(g_i · g_total) / (||g_i||_2 ||g_total||_2)`。 |
| `global/pre_reweight_full_grad_contribution/<domain_i>_to_pre_reweight_total/signed_projection_share` | reweight 前 domain gradient 对 pre-reweight total 方向的有符号投影贡献。 | `(g_i · g_total) / ||g_total||_2^2`；所有 domain 的 share 理论上相加为 1，负值表示该 domain 在抵消 pre-reweight total。 |
| `global/pre_reweight_full_grad_closure/domain_sum_vs_pre_reweight_audit_total/*` | domain vectors 之和与独立 pre-reweight audit total 的闭合误差。 | 输出 `cosine`、`diff_norm`、`rel_l2`、`norm_ratio` 和 `passed` 等。 |
| `<train_domain>/dynamic_weight/bounded_target_gradient_weight` | 最近一次 dynamic-weight update 根据 EMA gradient norm 算出的即时目标权重，已经过 mean-one projection 和 `[1/3, 3]` hard bounds，但尚未经过 applied-weight EMA。 | `BoundedMeanOne((mean(ema_norm) / ema_norm_i)^alpha)`。 |
| `<train_domain>/dynamic_weight/applied_gradient_weight` | applied-weight EMA 后真正用于 training backward 的 domain weight。 | `beta_w * previous_weight_i + (1 - beta_w) * bounded_target_i`，随后做 defensive bounded mean-one projection。 |
| `<train_domain>/dynamic_weight/ema_grad_norm` | 用于计算即时目标权重的 domain gradient-norm EMA。 | `beta_g * previous_ema_i + (1 - beta_g) * current_norm_i`。 |
| `<train_domain>/dynamic_weight/weighted_grad_norm` | 应用当前 dynamic weight 后的 domain gradient norm。 | `weight_i * ||g_i||_2`。 |
| `global/full_grad_training_parity/audit_total_vs_training_total/*` | audit reference 与实际 training gradient 的一致性；dynamic weighting 开启时两边均为 reweight 后口径。 | `cosine`、`norm_ratio`、`projection_share` 的理想值为 1；`diff_norm`、`rel_l2` 的理想值为 0；`passed` 的理想值为 1。 |
...
`full_grad_training_parity` 是两次 backward 的数值一致性检查，不应把所有字段都解读为“越接近 1 越好”。在 BF16 gradient snapshot、FSDP collective 和 FlashAttention2 下，即使两次 backward 使用相同 mask，replay 也可能出现小幅 reduction/rounding 差异，因此前三个 ratio/cosine 指标通常只是接近 1，`rel_l2` 通常只是接近 0。正式 BF16 smoke 配置使用 `full_grad_training_parity_rel_l2_threshold: 2.0e-2`；超过阈值或 `passed=0` 才表示需要进一步排查。需要验证数学上的严格等价时，应另跑 FP32、关闭非确定性 fused attention 的 control，而不是把诊断值强制截断为 1。
...
| `logp_vectors.jsonl` | 开启 `logp_vector_enabled` 后，每 step、每 train domain 的 signed teacher/student chosen-token log-prob gap occurrence vector。 |
| `logp_vocab_vectors.jsonl` | 开启 `logp_vector_enabled` 后，每 step、每 train domain 的 signed logp gap sum 与可选 occurrence-normalized mean vocab vectors。 |
| `logp_abs_vectors.jsonl` | 开启 `logp_abs_vector_enabled` 后，每 step、每 train domain 的 absolute logp gap occurrence vector。 |
| `logp_abs_vocab_vectors.jsonl` | 开启 `logp_abs_vector_enabled` 后，每 step、每 train domain 的 absolute logp gap sum 与可选 occurrence-normalized mean vocab vectors。 |
| `token_gradient_vocab_vectors.jsonl` | 开启 `token_gradient_log_tokens_jsonl_enabled` 后，每个 token-gradient audit step、每个 domain/selection 的 selected token occurrence count、signed configured-loss sum 和 absolute configured-loss sum dense vocab vectors。 |
