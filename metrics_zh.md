...
| Metric | 含义 | 计算方式 |
| --- | --- | --- |
| `<domain>/token_gap/gap_signed_*` | teacher chosen-token log-prob 与 student/old log-prob 的 signed gap 分布。 | 对该 domain 的有效 response token 计算 `gap_signed = teacher_logp - student_logp`，输出 mean/std/p05/p50/p95/max/sum；raw vector 写入 `token_gap_vectors.jsonl` 的 `gap_vector_domain`。 |
| `<domain>/token_gap/gap_abs_*` | signed gap 的绝对值分布。 | `gap_abs = abs(teacher_logp - student_logp)`，输出同一组分布统计；raw vector 写入 `gap_abs_vector_domain`。 |
...
token_gradient_max_samples_per_domain: 8
token_gradient_top_k_per_sample: 50
token_gradient_min_teacher_diff: 0.0
...
`token_gradient_max_samples_per_domain`、`token_gradient_top_k_per_sample` 和 `token_gradient_min_teacher_diff` 仅为兼容旧配置保留；当前 token-gradient audit 不再用它们截断候选池。
...
这组 metric 来自 `full_gradient_worker.py`，默认关闭。开启 `token_gradient_enabled: true` 后，tracker 会在命中 `token_gradient_freq_steps` 的 step 中，先按 domain 收集当前 step 全局所有 valid response token 的 `gap_abs = abs(teacher_logp - student_logp)` 分布，然后从这个全量分布中选择 `top100_gap_abs` 和覆盖 `token_gradient_top_p` 比例 gap_abs mass 的最小 token 集合，再对选中 token 做额外 gradient recompute：
...
| `<domain>/token_grad/selected_sample_count` | 当前 domain 被 `top100_gap_abs` 与 top-p 两个选择集合覆盖的样本数合计。 | 对两个 selection row 的 `selected_sample_count` 求和；两个集合重叠时会按 audit workload 计数。 |
| `<domain>/token_grad/selected_token_count` | 当前 domain 做 exact token gradient 的 token occurrence 数合计。 | `top100_gap_abs` 与 top-p 两个 selection row 的 token 数之和；重叠 token 会按 recompute workload 计数。 |
| `<domain>/token_grad/top100_gap_abs_selected_token_count` | 当前 domain 全局 `gap_abs` 最大的最多 100 个 token 数。 | 在全局候选分布上排序后取前 100。 |
...
| `<domain>/token_grad/top100_gap_abs_gap_abs_mass_frac` | top100 token 占当前 domain 全局 gap_abs mass 的比例。 | `top100_gap_abs_mass / global_candidate_gap_abs_mass`。 |
...
同时会写 `token_grad_metrics.jsonl`。每行是一个 selection summary，包含 `selection_scope=global`、`global_candidate_scope=all_valid_response_tokens`、`global_candidate_token_count`、`global_candidate_sample_count`、`selected_token_count`、`selected_gap_abs_mass_frac`、`own_domain_cos`、`other_domain_cos`、`conflict_to_other`、`own_projection_share` 和 `other_projection_share`。这份文件适合离线比较 top100 和 top-p token 集合与 domain gradient 的方向关系。
...
| `token_gap_vectors.jsonl` | 每 step、每 train domain 的 `teacher_logp - student_logp` 与 `abs(teacher_logp - student_logp)` raw token vectors。 |
