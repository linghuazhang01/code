    ...
    def __post_init__(self) -> None:
        ...
        for name, upper in bounds.items():
            ...
            if type(value) is not int or not 1 <= value <= upper:
                raise ValueError(f"teacher_performance.{name} must be an integer in 1..{upper}")
        if self.moe_dispatch not in {"stable_sort", "stock"}:
            raise ValueError("teacher_performance.moe_dispatch must be stable_sort or stock")
        ...
        if not math.isfinite(margin) or margin < 12:
            raise ValueError("teacher_performance.memory_margin_gib must be finite and >=12")
...
def teacher_performance_overrides(config: TeacherPerformanceConfig) -> list[str]:
    ...
    return [f"+actor_rollout_ref.ref.teacher_performance.{key}="
