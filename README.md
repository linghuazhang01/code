...
- `audit.token_gap_enabled=true` / `audit.token_gap_freq_steps=1`: records per-domain `teacher_logp - student_logp` and `abs(teacher_logp - student_logp)` distribution stats, plus raw per-occurrence response-token vectors in `token_gap_vectors.jsonl`. It also logs vocab-level domain-pair cosine scalars such as `global/token_gap_vocab_cosine/math_vs_code/gap_abs_sum_cosine` when response token ids are available. Set `audit.token_gap_vocab_vector_enabled=true` to additionally write dense vocab-indexed vectors to `token_gap_vocab_vectors.jsonl`; index `v` corresponds to token id `v`, with count, signed-gap sum, abs-gap sum, and mean vectors.
