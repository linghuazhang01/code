...
# Evaluation has separate protocol parsers. Keep this dispatch before the
# training parser so existing training semantics stay intact.
if [[ "${1:-}" == "--eval" ]]; then
  shift
  if [[ "${1:-}" == "--local" ]]; then
    shift
    exec bash "${SCRIPT_DIR}/scripts/run_local_math_parallel_eval.sh" "$@"
  fi
  [[ "${1:-}" != "--slurm" ]] || shift
...
Usage:
  ...
  bash start.sh --eval [--local] --model_path PATH [evaluation options]
...
Examples:
  ...
  bash start.sh --eval --local --model_path checkpoints/model --run_tag math_step60
