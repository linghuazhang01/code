...
class V3TopPSelectorCase:
    ...
    def filename(self) -> str:
        return (
            "a_next_step_expanded_pruned_v3_unified_"
            f"topp{self.slug}_i1_w1_{self.total_gpus}gpu_"
            f"{self.actor_gpus}a{self.ref_gpus}t_b{self.batch_size}.yaml"
        )

    @property
    def base_filename(self) -> str:
        return (
            "a_next_step_expanded_pruned_v3_unified_i1_w1_k25_"
            f"{self.total_gpus}gpu_{self.actor_gpus}a{self.ref_gpus}t_"
            f"b{self.batch_size}.yaml"
        )
    ...
    def run_id(self) -> str:
        return (
            f"q1p7b-math-a-ns-v3-u-topp{self.slug}-i1w1-"
            f"{self.total_gpus}gpu-{self.actor_gpus}a{self.ref_gpus}t-"
            f"b{self.batch_size}"
        )


TOP_P_CASES = tuple(
    V3TopPSelectorCase(slug, top_p, *topology)
    for slug, top_p in (
        ("0p01", 0.01),
        ("0p02", 0.02),
        ("0p05", 0.05),
...
def test_v3_math_next_step_top_p_config_contract(
    case: V3TopPSelectorCase,
) -> None:
    path = CONFIG_DIR / case.filename
    raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    config = load_config(path)
    command = format_command(build_command(config))
    candidate_ids = config.audit.domain_control_token_candidate_ids["math"]
    ...
    assert raw["extends"] == case.base_filename
    assert len(candidate_ids) == EXPECTED_MATH_V3_POOL_SIZE
    assert candidate_ids == sorted(set(candidate_ids))
    assert _pool_sha256(candidate_ids) == EXPECTED_MATH_V3_POOL_SHA256
    assert config.audit.execution_timing == "pre_update"
    assert config.audit.control_token_online_selection_enabled
    assert config.audit.control_token_online_selection_mode == "top_logp_diff"
    assert config.audit.control_token_online_weight_mode == "fixed"
    ...
    assert config.audit.control_token_online_top_k == 25
    assert config.audit.control_token_online_top_k_per_group is None
    ...
    assert config.worker_placement.ref_policy.n_gpus_per_node == case.ref_gpus
    assert config.trainer.n_gpus_per_node == case.actor_gpus
    assert config.runtime.wandb_run_id == case.run_id
    assert config.trainer.experiment_name == case.run_id
    assert config.audit.output_dir == f"audit/{case.run_id}"
    assert config.paper_eval.output_dir == f"eval_outputs/paper_suite/{case.run_id}"
    assert config.huggingface_checkpoint.path_prefix == (
        f"checkpoints/math/{case.run_id}"
    )
    assert config.trainer.default_local_dir == f"checkpoints/MOPD/{case.run_id}"
    assert len(config.runtime.wandb_run_id or "") <= 64
    ...
    assert "+mopd_audit.control_token_online_top_k=25" in command
    assert "+mopd_audit.domain_control_token_candidate_ids=" in command
    fsdp_override = "actor_rollout_ref.ref.fsdp_config.fsdp_size=2"
    assert (fsdp_override in config.extra_overrides) == (case.ref_gpus == 2)


def test_v3_math_next_step_top_p_paths_are_unique() -> None:
    configs = [load_config(CONFIG_DIR / case.filename) for case in TOP_P_CASES]

    run_ids = [config.runtime.wandb_run_id for config in configs]
    audit_dirs = [config.audit.output_dir for config in configs]
    eval_dirs = [config.paper_eval.output_dir for config in configs]
    hf_prefixes = [
        config.huggingface_checkpoint.path_prefix for config in configs
    ]
    checkpoint_dirs = [config.trainer.default_local_dir for config in configs]
    assert len(run_ids) == len(set(run_ids)) == len(TOP_P_CASES)
    assert len(audit_dirs) == len(set(audit_dirs)) == len(TOP_P_CASES)
    assert len(eval_dirs) == len(set(eval_dirs)) == len(TOP_P_CASES)
    assert len(hf_prefixes) == len(set(hf_prefixes)) == len(TOP_P_CASES)
    assert len(checkpoint_dirs) == len(set(checkpoint_dirs)) == len(TOP_P_CASES)
