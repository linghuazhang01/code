...
现在保留两个正式 MOPD 版本，每个版本提供 2/4/8 卡配置；另外保留一份指标 smoke profile：
...
`mopd_formal_audit_all_smoke.yaml` 作为指标测试 profile 保留并纳入测试。它使用 `data.train_batch_size=32`、`actor.ppo_mini_batch_size=32`、`trainer.total_training_steps=1`，但保持正式 `data.max_response_length=16384`，并保留 full-vocab token gap 与 entropy vectors。
...
使用 `mopd_formal_audit_all_*gpu.yaml` 时，JSONL audit 文件写入对应目录，例如 `audit/formal_audit_all_2gpu/`。
